...
def load_data(time_window, config, data_type, clinicalBERT, embedding_type):
    ...
    outputs = {
            ...
            'num_classes' : 2,
            'total_data' : df,
            'input_size' : embedding_size, # - episode id, - sepsisBin
            'row_df' : row_df,
...
def load_batch_data(config, data_from_tune_file, batch_size=200):
    
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    ep_id_list = data_from_tune_file['episode_id_list']
    df = data_from_tune_file['total_data']
