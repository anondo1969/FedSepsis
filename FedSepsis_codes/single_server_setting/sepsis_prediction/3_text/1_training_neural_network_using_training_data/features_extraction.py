...
def load_train_data(time_window, config, clinicalBERT, embedding_type):
    ...
    #try:
        ...
        if clinicalBERT == 'Emily':
            ...
            embedding_dict = torch.load(embedding_data_dir+'clinicalBERT_Huang_torch_'+embedding_type+'_embedding_dict')
...
def load_train_batch_data(ep_id_list, whole_data, row_df, embedding_dict, embedding_size, label_column, all_pos_all_neg):
    ...
    for ep_id in ep_ids:
        ...
        for one_sequence in seq:
            ...
            #input_seq.append(embedding_sequence_tensor.cpu().detach().numpy())
    ...
    #input_seq_tensor=torch.stack(input_seq_list).to(device)
    #output_seq_tensor=torch.stack(output_seq_list).to(device)
    #return input_seq_tensor, output_seq_tensor, max_seq_length
