...
def load_decoding_details(config, value, time_window, change, time_stamp):
    ...
    try:
        ...
        with open(save_dir+str(time_window)+'_hours_'+change+"_"+time_stamp+"_test_decoding_details_"+str(value), "rb") as fp:
            decoding_details = pickle.load(fp)
        os.remove(save_dir+str(time_window)+'_hours_'+change+"_"+time_stamp+"_test_decoding_details_"+str(value))   
...
def write_experiment_information(config, average_prior_prediction_in_hour, episode_wise_evaluation_scores, settings, range_limit, value, time_window, change, header, clinicalBERT, embedding_type, batch_size, time_stamp, all_hyper_parameters):
    ...
    experiment_wise_information.append(str(all_hyper_parameters[value-1][3]))  
    experiment_wise_information.append(str(all_hyper_parameters[value-1][4])) 
    
    if all_hyper_parameters[value-1][4]==1:
        experiment_wise_information.append('0')
    else:
        experiment_wise_information.append(str(all_hyper_parameters[value-1][5])) 
    experiment_wise_information.append(str(all_hyper_parameters[value-1][6])) 
    ...
    experiment_wise_information.append('no')
