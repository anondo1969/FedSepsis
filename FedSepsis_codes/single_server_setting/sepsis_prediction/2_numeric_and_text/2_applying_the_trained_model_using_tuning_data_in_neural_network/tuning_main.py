...
print('\nTuning: Experiment Number: '+str(experiment_number))

#training
data_from_tune_file = features_extraction.load_data(time_window, imputation, config, 'tune', clinicalBERT, embedding_type)
...
model = neural_network_testing_v2.load_trained_model(config, custom_parameters, data_from_tune_file, experiment_number, time_window, imputation, change, time_stamp)


decoding_details = neural_network_testing_v2.testing_neural_network_model(batch_size, config, data_from_tune_file, model, experiment_number, all_input_seq_list, all_output_seq_list, all_max_seq_length, batch_wise_ep_ids, time_window, imputation, change, time_stamp)
