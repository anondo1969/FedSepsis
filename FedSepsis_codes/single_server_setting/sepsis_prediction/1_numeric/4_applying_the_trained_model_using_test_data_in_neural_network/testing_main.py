...
for all_pos_all_neg in [1, 0]:
    ...
    best_tune_experiments = dict()
    for imputation in imputation_list:
                           
        imputation_df = tune_df.loc[(tune_df['imputation'] == imputation) & (tune_df['settings'] == 1) & (tune_df['range_limit'] == 0) & (tune_df['training_label_change'] == change)]
        
        sorted_imputation_df = imputation_df.sort_values(by=[sorting_column], ascending=False)
        
        best_tune_experiments[imputation]=sorted_imputation_df.iloc[0][0]
    ...
    for time_window in [1]:
        ...
        for imputation in ['imputed', 'non_imputed', 'GAN_imputed_10_alpha', 'GAN_imputed_100_alpha']:
            ...
            data_from_test_file = features_extraction.load_data(time_window, imputation, config, 'test')
            ...
            experiment_number = best_tune_experiments[imputation]-1
            hyper_parameters = all_hyper_parameters[experiment_number]
            
            #print('\nExperiment Number: '+str(experiment_number+1))
            #print('alpha\tbeta_one\tbeta_two\thidden_units\thidden_layers\tdrop_out\tepochs')
            #print(hyper_parameters)
            #print()
        
            model = neural_network_testing_v2.load_trained_model(config, hyper_parameters, data_from_test_file, experiment_number+1, time_window, imputation, change, time_stamp)
        

            decoding_details = neural_network_testing_v2.testing_neural_network_model(batch_size, config, data_from_test_file, model, experiment_number+1, all_input_seq_list, all_output_seq_list, all_max_seq_length, batch_wise_ep_ids, time_window, imputation, change, time_stamp)
