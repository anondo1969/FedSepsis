...
def testing_neural_network_model(batch_size, config, data_from_test_file, model, exp_no, all_input_seq_list, all_output_seq_list, all_max_seq_length, all_batch_wise_ep_ids, time_window, imputation, change, time_stamp):
    ...
    with open(save_dir+str(time_window)+'_hours_'+imputation+'_'+change+"_"+time_stamp+"_test_decoding_details_"+str(exp_no), "wb") as fp:
        pickle.dump(decoding_details, fp)
            
    print('Test data applied successfully in the trained model')
...
def load_trained_model(config, hyper_parameters, data_from_test_file, experiment_number, time_window, imputation, change, time_stamp):
    ...
    num_classes = data_from_test_file['num_classes']+1 # additional one is for the dummy class in padding (0)
