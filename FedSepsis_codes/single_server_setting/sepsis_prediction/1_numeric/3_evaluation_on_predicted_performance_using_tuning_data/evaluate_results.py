...
def load_decoding_details(config, value, time_window, imputation, change, time_stamp):
    ...
    try:
        ...
        with open(save_dir+str(time_window)+'_hours_'+imputation+'_'+change+"_"+time_stamp+"_decoding_details_"+str(value), "rb") as fp:
            decoding_details = pickle.load(fp)
        os.remove(save_dir+str(time_window)+'_hours_'+imputation+'_'+change+"_"+time_stamp+"_decoding_details_"+str(value))
...
def write_experiment_information(config, average_prior_prediction_in_hour, episode_wise_evaluation_scores, settings, range_limit, value, time_window, imputation, change, header, batch_size, time_stamp, all_hyper_parameters):
