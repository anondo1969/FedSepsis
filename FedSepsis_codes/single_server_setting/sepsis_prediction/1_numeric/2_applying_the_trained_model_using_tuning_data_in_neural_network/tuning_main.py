...
for all_pos_all_neg in [1, 0]:
    ...
    for time_window in [1]:
        ...
        for imputation in ['imputed', 'non_imputed', 'GAN_imputed_10_alpha', 'GAN_imputed_100_alpha']:
            ...
            data_from_tune_file = features_extraction.load_data(time_window, imputation, config, 'tune')
            ...
            for experiment_number in range(len(all_hyper_parameters)):
                ...
                model = neural_network_testing_v2.load_trained_model(config, hyper_parameters, data_from_tune_file, experiment_number+1, time_window, imputation, change, time_stamp)
        

                decoding_details = neural_network_testing_v2.testing_neural_network_model(batch_size, config, data_from_tune_file, model, experiment_number+1, all_input_seq_list, all_output_seq_list, all_max_seq_length, batch_wise_ep_ids, time_window, imputation, change, time_stamp)
